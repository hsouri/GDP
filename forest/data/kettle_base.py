...
class _Kettle():
    ...
    def __init__(self, args, batch_size, augmentations, mixing_method=dict(type=None, strength=0.0),
        ...
        self.normalize_ = not self.args.nonormalize

        self.trainset, self.validset = construct_datasets(self.args.dataset, self.args.data_path, self.normalize_)
        if self.args.pretrain_dataset is not None:
            self.pretrain_trainset, self.pretrain_validset = construct_datasets(self.args.pretrain_dataset,
                                                                                self.args.data_path, self.normalize_)
        self.prepare_diff_data_augmentations(normalize=self.normalize_)
