...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name or 'GTSRB' in dataset_name:
        if pretrained:
        ...
        if 'VGG' in model_name:
            model = VGG(model_name, in_channels=in_channels, num_classes=num_classes, head='TinyImageNet')
        ...
        raise ValueError(f'No models registered for dataset {dataset_name}.')
...
def linear_model(dataset, num_classes=10):
    ...
    if 'cifar' in dataset.lower() or 'gtsrb' in dataset.lower():
        dimension = 3072
