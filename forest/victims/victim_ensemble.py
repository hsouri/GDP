...
class _VictimEnsemble(_VictimBase):
    ...
    def gradient(self, images, labels, criterion=None):
        ...
        grad_list, norm_list = [], []
        for model in self.models:
            with GPUContext(self.setup, model, keep_device=self.args.diffusion_base_poisons) as model:
    ...
    def compute_diff(self, function, *args):
        """Compute function on all models.
        ...
        for idx, (model, optimizer) in enumerate(zip(self.models, self.optimizers)):
            with GPUContext(self.setup, model, keep_device=self.args.diffusion_base_poisons) as model:
                ...
                outputs.append(function(model, optimizer, *single_arg))
        # collate
        # the following is cpmpatible with the diffusion poisoning as it needs the computational graph:
        avg_output = tuple(torch.stack(tensors).float().mean(dim=0) for tensors in zip(*outputs))
